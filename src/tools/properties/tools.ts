...
export class GetPropertiesToolHandler extends BaseToolHandler<GetPropertiesArgs> {
	...
	async runTool(args: GetPropertiesArgs): Promise<Array<TextContent>> {
		...
		try {
			...
			logger.error(
				`Error getting properties from ${args.filepath}:`,
				errorToObject(error)
			);
...
export class UpdatePropertiesToolHandler extends BaseToolHandler<UpdatePropertiesArgs> {
	...
	getToolDescription(): Tool {
		return {
			name: this.name,
			description:
				"Update properties in an Obsidian note's YAML frontmatter. Intelligently merges arrays (tags, type, status), handles custom fields, and automatically manages timestamps. Valid property types:\n- type: Any string value\n- status: ['draft', 'in-progress', 'review', 'complete']\n- tags: Array of strings starting with '#'\n- Other fields: title, author, version, platform, repository (URI), dependencies, sources, urls (URI), papers",
			examples: [
				{
	...
	async runTool(args: UpdatePropertiesArgs): Promise<Array<TextContent>> {
		...
		try {
			...
			logger.error(
				`Error updating properties for ${args.filepath}:`,
				errorToObject(error)
			);
			return this.handleError(error);
		}
